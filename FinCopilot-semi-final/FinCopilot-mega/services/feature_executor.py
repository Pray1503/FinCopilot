...
def _try_bill_summary() -> dict | None:
    """Fetch scanned bill summary from the OCR database."""
    try:
        ...
        return {
            ...
            "health_score": score,
            "insights": insights,
            "top_categories": {k: round(v) for k, v in top_cats.items()},
        }
    except Exception:
        return None
...
def _format_feature_result(feature_data: dict) -> str:
    ...
    if ft == "boardroom":
        ...
        m = feature_data["metrics"]
        top_cats = feature_data.get("top_categories", {})
        cats_html = " ".join(
            f'<div class="result-metric"><div class="label">{cat}</div><div class="value">₹{amt:,}</div></div>'
            for cat, amt in list(top_cats.items())[:3]
        )
        html = f"""
        <div class="result-inline">
            <div style="font-weight:700;font-size:1rem;margin-bottom:8px;">📄 Bill Scanner Summary</div>
            <div style="display:flex;flex-wrap:wrap;gap:6px;margin:8px 0;">
                <div class="result-metric"><div class="label">Total Scanned</div><div class="value">₹{m['total']:,.0f}</div></div>
                <div class="result-metric"><div class="label">Bills</div><div class="value">{m['count']}</div></div>
                <div class="result-metric"><div class="label">Average</div><div class="value">₹{m['average']:,.0f}</div></div>
                <div class="result-metric"><div class="label">Highest</div><div class="value">₹{m['highest']:,.0f}</div></div>
            </div>
            {f'<div style="margin-top:8px;font-size:0.85rem;color:#94a3b8;">Top Categories:</div><div style="display:flex;flex-wrap:wrap;gap:6px;margin:4px 0;">{cats_html}</div>' if cats_html else ''}
        </div>"""
        return html
...
def execute_query(query: str, profile: dict | None = None) -> dict:
    ...
    if intent in ("budget", "loan_check", "risk", "goal"):
        ...
        feature_data = _try_bill_summary()
        if feature_data:
            feature_html = _format_feature_result(feature_data)
            m = feature_data["metrics"]
            context_for_llm = (
                f"\n\n[BILL SCANNER DATA]\n"
                f"Total bills scanned: {m['count']}\n"
                f"Total amount: ₹{m['total']:,.0f}\n"
                f"Average bill: ₹{m['average']:,.0f}\n"
                f"Highest bill: ₹{m['highest']:,.0f}\n"
                f"Insights: {' | '.join(feature_data.get('insights', []))}\n"
            )
        else:
            feature_html = (
                '<div class="feature-card">'
                '<h5>📄 Bill Scanner Ready</h5>'
                '<p>Navigate to the <strong>Bill Scanner</strong> page from the sidebar to upload and scan your receipts! '
                'OCR will extract vendor, amount, date, and auto-categorize your expenses.</p>'
                '</div>'
            )
